    ...
    button.addEventListener("click", () => {
        if (currentInput.length < maxInputLength) {
            if (currentInput === "0" && button.textContent === "0") {
                // Eğer currentInput sadece 0 ise ve kullanıcı 0'a basarsa hiçbir şey yapma
                return;
            } else if (currentInput === "0") {
                // Eğer currentInput sadece 0 ise ve kullanıcı başka bir rakama basarsa sadece yeni rakamı göster
                currentInput = button.textContent;
            } else {
                // Diğer durumlarda, rakamı currentInput'e ekle
                currentInput += button.textContent;
            }
            // Başındaki sıfırları kaldır
            currentInput = currentInput.replace(/^0+/, "");
            // Eğer currentInput boşsa, currentInput 0 olsun
            if (currentInput === "") {
                currentInput = "0";
            }
            updateDisplay();
